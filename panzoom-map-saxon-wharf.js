(() => {
  ...
  const GAP = -4;
  const HOME_INITIAL_ZOOM = 1;
  const HOME_INITIAL_VERTICAL_OFFSET = 0;
  const APARTMENT_INITIAL_ZOOM = 1;
  ...
  function applyInversePinScale() {
    ...
    document.querySelectorAll(`${MAP_CONTAINER_SEL} ${PIN_WRAP_SEL}`).forEach((pin) => {
      const isRightAligned = pin.dataset.pinList === "list-2";
      const horizontalOffset = isRightAligned ? "-100%" : "-50%";
      pin.style.transformOrigin = isRightAligned ? "100% 50%" : "50% 50%";
      pin.style.transform = `translate(${horizontalOffset}, -50%) scale(${inv})`;
    });
  }
  ...
  function buildPins() {
    ...
    items.forEach((item) => {
      ...
      wrap.dataset.pinList = item.closest(LIST_SEL)?.getAttribute("pin") ||
        item.closest(LIST_SEL)?.getAttribute("pins") || "list";
      wrap.style.position = "absolute";
      wrap.style.left = `${x}%`;
      wrap.style.top = `${y}%`;
      wrap.style.transformOrigin = wrap.dataset.pinList === "list-2" ? "100% 50%" : "50% 50%";
      wrap.style.transform = wrap.dataset.pinList === "list-2"
        ? "translate(-100%, -50%)"
        : "translate(-50%, -50%)"; // scale applied after build
